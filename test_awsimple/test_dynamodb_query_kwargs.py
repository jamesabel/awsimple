...
def test_dynamodb_query_kwargs():
    ...
    response = dynamodb_access.query_one(partition_value="me")  # highest is default
    assert response["answer"] == 99
    assert response["name"] == "zzz"  # the "last" entry, as sorted by sort key

    response = dynamodb_access.query_one(partition_value="me", direction=QuerySelection.highest)
    assert response["answer"] == 99
    assert response["name"] == "zzz"  # the "last" entry, as sorted by sort key

    response = dynamodb_access.query_one(partition_value="me", direction=QuerySelection.lowest)
    assert response["answer"] == 13
    assert response["name"] == "james"  # the "first" entry, as sorted by sort key

    response = dynamodb_access.query_one(partition_value="idonotexist", direction=QuerySelection.lowest)
    assert response is None


if is_main():
    test_dynamodb_query_kwargs()
