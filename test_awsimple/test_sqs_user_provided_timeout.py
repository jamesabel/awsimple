...
def test_user_provided_minimum_timeout():
    ...
    q = SQSPollAccess(test_awsimple_str, minimum_timeout=round(10.0*work_time), immediate_delete=False, profile_name=test_awsimple_str)
    q.send(send_message)
    time.sleep(1.0)
    receive_message = q.receive_message()
    assert receive_message.message == send_message
...
def test_actually_timeout():
    ...
    q = SQSPollAccess(test_awsimple_str, timeout=round(0.5 * work_time), immediate_delete=False, profile_name=test_awsimple_str)
    q.send(send_message)
    time.sleep(1.0)
    receive_message = q.receive_message()
